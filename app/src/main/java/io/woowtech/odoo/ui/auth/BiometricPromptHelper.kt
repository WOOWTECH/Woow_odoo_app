 ...
 * [BiometricScreen] can stay in pure Compose code and the whole surface can be unit-tested
 * by swapping this class for a fake.
 *
 * Enforces `BIOMETRIC_STRONG` only — we do not silently downgrade to `BIOMETRIC_WEAK`.
 * PBKDF2/PIN is the intended fallback when strong biometrics are unavailable. This matches
 * the semantics of iOS's `LAPolicy.deviceOwnerAuthenticationWithBiometrics`.
 *
...
class BiometricPromptHelper(
    ...
    private val promptInfoFactory: (String, String, String) -> BiometricPrompt.PromptInfo =
        ::defaultPromptInfoFactory,
) {
    ...
    /**
     * Shows the system biometric prompt with an optional [BiometricPrompt.CryptoObject] binding.
     *
     * Requiring a CryptoObject ensures that a successful callback cannot be spoofed:
     * the AndroidKeyStore key is only released to the cipher after a genuine biometric match,
     * so `onSuccess` can only fire if the hardware validated the biometric AND the cipher was
     * successfully initialised by the Keystore.
     *
     ...
     * material involved.
     *
     * - [onSuccess]: biometric matched; if a CryptoObject was supplied, the cipher is
    ...
    fun prompt(
        ...
        val callback = object : BiometricPrompt.AuthenticationCallback() {
            override fun onAuthenticationSucceeded(result: BiometricPrompt.AuthenticationResult) {
                ...
                if (cryptoObject != null && result.cryptoObject?.cipher == null) {
                    Timber.w("BiometricPrompt returned success but cipher is null — treating as failure")
                    onFailed()
                    return
                }
                onSuccess(result)
        ...
        val info = promptInfoFactory(title, subtitle, negativeText)
...
/**
 * Default implementation that builds a [BiometricPrompt.PromptInfo] using the platform
 * [BiometricPrompt.PromptInfo.Builder]. Extracted so tests can supply a mock without
 * invoking [android.text.TextUtils.isEmpty] which is not available on the JVM test runtime.
 */
private fun defaultPromptInfoFactory(
    title: String,
    subtitle: String,
    negativeText: String,
