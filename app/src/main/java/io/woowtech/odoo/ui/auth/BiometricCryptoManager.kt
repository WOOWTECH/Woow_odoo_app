 ...
 *
 * Usage pattern:
 * 1. Call [getEncryptCipher] → pass the resulting [Cipher] inside a [BiometricPrompt.CryptoObject]
 * 2. On [BiometricPrompt.AuthenticationResult], verify `result.cryptoObject?.cipher != null`
...
class BiometricCryptoManager {
    ...
    fun getEncryptCipher(): Cipher {
        ...
        return Cipher.getInstance(AES_GCM_NO_PADDING).also {
            try {
                ...
                Log.w(TAG, "Biometric key permanently invalidated — deleting key")
                deleteKey()
                throw e
            }
        }
    ...
    fun getDecryptCipher(iv: ByteArray): Cipher {
        ...
        return Cipher.getInstance(AES_GCM_NO_PADDING).also {
            try {
                ...
                Log.w(TAG, "Biometric key permanently invalidated during decrypt — deleting key")
                deleteKey()
                throw e
            }
        }
    ...
    fun deleteKey() {
        try {
            ...
            Log.e(TAG, "Failed to delete biometric Keystore key", e)
        }
    }
    ...
    companion object {
        ...
        private const val TAG = "BiometricCryptoManager"
    }
}
